...
jest.mock('expo-router', () => ({
  useRouter: () => ({ navigate: jest.fn(), push: jest.fn(), back: jest.fn(), replace: jest.fn(), canGoBack: jest.fn(() => true) }),
}));
...
describe('SettingsScreen', () => {
  ...
  test('renders Account section header', () => {
    const element = (SettingsScreen as any)() as unknown as MockElement;
    const headers = findElements(element, (el) => el.props?.title === 'Account');
    expect(headers.length).toBeGreaterThan(0);
  });

  test('renders sign-in-to-sync row', () => {
    const element = (SettingsScreen as any)() as unknown as MockElement;
